                ...
                if 'external_id' in reference:
                    if ((reference['external_id'].startswith("T"))):
                        ...
                        if 'x_mitre_deprecated' in object:
                            ...
                            filtered_object = {'tactics': str(
                                tactics), 'technique': technique, 'name': name, 'url': url, 'deprecated': deprecated}
                            mitreMapped[technique] = filtered_object
                        else:
                            filtered_object = {'tactics': str(
                                tactics), 'technique': technique, 'name': name, 'url': url, 'deprecated': "False"}
                            mitreMapped[technique] = filtered_object
...
for root, dirs, files in os.walk(
        "detections/"):  # directory of choice
    for file in files:
                ...
                if alert['rule']['threat'][0]['framework'] == "MITRE ATT&CK":
                    for threat in alert['rule']['threat']:
                        ...
                        # else:
                            ...
                            filtered_object = {'tactics': tactic, 'technique_id': technique_id, "technique_name": technique_name,
                                               "subtechnique_id": subtechnique_id, "subtechnique_name": subtechnique_name}
...
# check MITRE tactics. remove none to enforce
mitre_tactic_list = ['none', 'reconnaissance', 'resource development', 'initial access', 'execution', 'persistence', 'privilege escalation',
